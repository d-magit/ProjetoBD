...
BEGIN
    ...
    CREATE VIEW Join_Pol_Gov AS
        ...
        FROM Politico AS po INNER JOIN Governador AS Gov ON po.ID = Gov.Politico_ID;
    ...
    CREATE VIEW Join_Gov_UF AS
        ...
        FROM Join_Pol_Gov AS Gov INNER JOIN Unidade_Federativa AS uf ON Gov.UF_Codigo = uf.Codigo;
    
    CREATE VIEW Join_Gov_UF_Pais AS
        SELECT Gov.*, p.Nome AS Pais, p.Continente, p.PIB as PIB_Pais 
        FROM Join_Gov_UF AS Gov INNER JOIN Pais AS p ON Gov.Pais_Codigo = p.Codigo;
...
BEGIN
    ...
    CREATE VIEW Join_Pol_Dep AS
        ...
        FROM Politico AS po INNER JOIN Deputado AS Dep ON po.ID = Dep.Politico_ID;
    ...
    CREATE VIEW Join_Dep_UF AS
        ...
        FROM Join_Pol_Dep AS Dep INNER JOIN Unidade_Federativa AS uf ON Dep.UF_Codigo = uf.Codigo;
    
    CREATE VIEW Join_Dep_UF_Pais AS
        SELECT Dep.*, p.Nome AS Pais, p.Continente, p.PIB as PIB_Pais 
        FROM Join_Dep_UF AS Dep INNER JOIN Pais AS p ON Dep.Pais_Codigo = p.Codigo;
...
BEGIN
    ...
    CREATE VIEW Join_Min_Mis_Pais AS
        ...
        FROM Join_Min_Mis AS Min INNER JOIN Pais AS p ON Min.Pais_Codigo = p.Codigo;
...
BEGIN
    ...
    CREATE VIEW Join_Pol_Pres AS
        ...
        FROM Politico AS po INNER JOIN Presidente AS pr ON po.ID = pr.Politico_ID;
    
    CREATE VIEW Join_Pres_Pais AS
		SELECT Pres.*, p.Nome AS Pais, p.Continente, p.PIB as PIB_Pais 
        FROM Join_Pol_Pres AS Pres INNER JOIN Pais AS p ON Pres.Pais_Codigo = p.Codigo;
...
BEGIN
    ...
    SET @Gov_AVG = (SELECT AVG(Salario) FROM Join_Gov_UF_Pais AS j WHERE j.Pais_Codigo = Pais_Codigo);
    SET @Dep_AVG = (SELECT AVG(Salario) FROM Join_Dep_UF_Pais AS j WHERE j.Pais_Codigo = Pais_Codigo);
    SET @Min_AVG = (SELECT AVG(Salario) FROM Join_Min_Mis_Pais AS j WHERE j.Pais_Codigo = Pais_Codigo);
    SET @Pres_AVG = (SELECT AVG(Salario) FROM Join_Pres_Pais AS j WHERE j.Pais_Codigo = Pais_Codigo);
